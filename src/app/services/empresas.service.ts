...
export class EmpresasService {
    ...
    // OBTENER ID POR EL ROL EMPRESA
    rolEmpresaId(id: String, token): Observable<any> {
      let headersToken=this.headersVariable.set('Authorization', token)
      return this._http.get(this.url + '/rolEmpresaId/' + id, { headers: headersToken })
    }


    // EDITAR POR EL ROL EMPRESA
    editarUsuario(modeloEmpresa: Empresas, token): Observable<any> {

      let parametros = JSON.stringify(modeloEmpresa);

      let headersToken = this.headersVariable.set('Authorization', token);

      return this._http.put(this.url + '/editarUsuario/' + modeloEmpresa._id, parametros, { headers: headersToken })
    }
